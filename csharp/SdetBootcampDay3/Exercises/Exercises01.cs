...
namespace SdetBootcampDay3.Exercises
{
    ...
    public class Exercises01
    {
        ...
        public async Task GetDataForUser4_CheckName_ShouldBePatriciaLebsack()
        {
             ...
             */
            string expectedName = "Patricia Lebsack";
            RestRequest request = new RestRequest("/users/4");
            RestResponse response = await client.ExecuteAsync(request);
        ...
        public async Task GetDataForUser5_CheckCompanyName_ShouldBeKeeblerLLC()
        {
             ...
             */
            string expectedCompany = "Keebler LLC";
            RestRequest request = new RestRequest("/users/5");
            RestResponse response = await client.ExecuteAsync(request);

            JObject responseData = JObject.Parse(response.Content!);

            Assert.That(responseData.SelectToken("company.name")!.ToString(), Is.EqualTo(expectedCompany));
        }
    }
}
